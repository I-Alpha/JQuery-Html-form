 ...
 function ProcessExcel(data) {
     ...
     for (var i = 1; i < excelRows.length; i++) {
         ...
         postData.push(JSON.stringify(Qobject));
...
function toggleCheckState(text, elem) {
 ...
 $(document).ready(function () {
     ...
     $("form").submit(function (e) {
         ...
         if($("vis").html() ==  "" || $("vis").html() ==  " ")
           {e.preventDefault()
             alert("Please complete additional questions underneath questions!").done()
            }
     ...
     $("input.skipCheck").change(function(e){
        ...
        var skiptextElemtxtArea =Row.find(".questCol span textarea")
        var rowID=Row.data(`rownum`);

        if($(this).is(" :checked")){
            skiptextElem.show()
              skiptextElem.addClass("vis")}
        else{
           skiptextElem.hide()
           skiptextElem.removeClass("vis");}

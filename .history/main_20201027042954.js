...
function processExcel(data) {
    ...
    for (var i = 1; i < excelRows.length; i++) {
        ...
        tableRow.append(question_td, inputCols_td,alt)
...
function checkSubmit() {
    ...
    var allcheckboxes = document.getElementsByClassName('input');
    //simple check if every visible text area .required has been filled out. if it is enable to the out 
    for (let i = 0; i < allcheckboxes.length; i++) {
        if (allcheckboxes[i].className == "required" || allcheckboxes[i].className == "vis") {
            if (allcheckboxes[i].value == "" || allcheckboxes[i].value == " " || allcheckboxes[i].html=="") {
                (submitButton.hasClass("disabled"))

...
function ProcessExcel(data) {
    ...
    //Add the label cell
    var questionLabel = $("#hidden-template .questionLabel").clone(true,true);

    //Add input block 
    var inputBlock = $("#hidden-template  .inputContainer").clone(true,true)
    ...
    for (var i = 1; i < excelRows.length; i++) {
        ...
        var formRow = $("#form1");
        formRow.css("border-bottom","1px solid black");
        //GET DATA FROM EXCEL ROW
        currentRow = excelRows[i];
        //get Question text
        questionLabel.html(currentRow.Standard)
        //add QuestionText to HTML ROW
        tableRow.append(questionLabel)
        tableRow.append(inputBlock)

        ...
        $.ajax({
            ...
            success: function(jqh) {
              ...
              if (jqh == 202 || jqh == 200) {
                ...
                window.location.href = "https://qbe.sharepoint.com/sites/EPMO"
              } else {
                  alert("There's been an error, please, try submitting again or contact the PWA Admins. errorcode = " + jqh)
              }
            }
           ,
            error: (function(jqHXR,exception){function(){alert(`It seems there's an error. ${jqHXR.status}. Hmm. Email this JSON to this address.Thanks!`), download(JSON.stringify(postData), 'json.txt', 'text/plain')})
    }); 
})
